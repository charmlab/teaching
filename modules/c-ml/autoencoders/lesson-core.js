/* ══════════════════════════════════════════════════════════════
   lesson-core.js — shared framework for the Autoencoders & VAEs lesson
   Canvas/plot helpers, math (real computation, nothing staged),
   quiz engine, Python highlighter, and lazy figure mounting.
   Same LR namespace and helper names as the other ML Bible lessons.
   ══════════════════════════════════════════════════════════════ */
(function () {
  "use strict";
  ...
  /* ── math: linear algebra used across the figures ───────── */
  ...
  // Returns {values, vectors} with values sorted descending;
  // vectors[k] is the unit eigenvector for values[k].
  LR.eigSym = function (Ain) {
    const n = Ain.length;
  ...
  // covariance matrix of an array of D-dim points (rows), 1/N convention
  LR.cov = function (pts) {
    const n = pts.length, d = pts[0].length;
    const mean = new Array(d).fill(0);
    pts.forEach((p) => p.forEach((v, j) => (mean[j] += v / n)));
    const C = Array.from({ length: d }, () => new Array(d).fill(0));
    pts.forEach(function (p) {
      for (let i = 0; i < d; i++)
        for (let j = 0; j < d; j++)
          C[i][j] += ((p[i] - mean[i]) * (p[j] - mean[j])) / n;
    });
    return { mean, C };
  };
